...
public class VectorStoreService {
  ...
  /**
   * Stores a document in the vector store. Spring AI automatically generates embeddings and
   * persists to PostgreSQL.
   */
  public void store(Document document) {
    log.debug(
        "Storing document {} for source: {}",
        document.getId(),
        document.getMetadata().get("documentName"));
   ...
   * @param documentIds Optional list of document IDs to restrict search. If null or empty, searches
   *     all documents.
   * @return List of matching documents
   */
  public List<Document> search(String query, List<String> documentIds) {
    log.debug(
        "Searching for similar documents (maxResults={}, minScore={}, documentIds={})",
        maxResults,
        minScore,
        documentIds);

    SearchRequest.Builder requestBuilder =
        SearchRequest.builder().query(query).topK(maxResults).similarityThreshold(minScore);
    ...
    if (documentIds != null && !documentIds.isEmpty()) {
      ...
      if (filter != null) {
        ...
        log.debug("Applied document filter for {} document IDs", documentIds.size());
      }
    }
  ...
  /**
   * Removes all documents associated with a source document ID. Uses filter expression to delete
   * from PostgreSQL by metadata.
   */
  ...
  /**
   * Updates the document name in vector store metadata for all chunks of a document. Uses native
   * SQL to update the JSON metadata column in PostgreSQL. Note: The vector_store table uses JSON
   * type (not JSONB), so we cast to JSONB, update, and cast back.
   *
  ...
  public void updateDocumentName(String documentId, String newDocumentName) {
    ...
    log.info(
        "Updated document name in {} vector store entries for documentId: {}",
        updatedRows,
        documentId);
  }
}
